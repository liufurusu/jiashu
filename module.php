...
function blog_navi(){
	...
	foreach($navi_cache as $value):
        ...
        $current_tab = BLOG_URL . trim(Dispatcher::setPath(), '/') == $value['url'] ? '' : '';
		?>
		 <?php echo $current_tab;?>
			<a href="<?php echo $value['url']; ?>" <?php echo $newtab;?>title="<?php echo $value['naviname']; ?>"><i class="fa fa-<?php echo $value['naviname']; ?>"></i><span><?php echo $value['naviname']; ?></span></a>
			<?php if (!empty($value['children'])) :?>
            <ul class="sub-nav">
...
function blog_comments($comments){
	...
	<ol class="comment-list"><li itemscope="" itemtype="http://schema.org/UserComments" id="comment-<?php echo $comment['cid']; ?>" class="comment-body comment-parent comment-odd comment-by-author">
    <div class="comment-avatar">
		<?php if($isGravatar == 'y'): ?><div class="avatar"><img src="<?php echo getGravatar($comment['mail']); ?>" /></div><?php endif; ?>
	</div>
    <div class="comment-meta">
        <div class="comment-meta-author"><?php echo $comment['poster']; ?> </div>
        <div class="comment-meta-time">
        <a href="#comment-<?php echo $comment['cid']; ?>"><time itemprop="commentTime" datetime="2016-04-26T20:45:39+08:00"><?php echo $comment['date']; ?></time></a>
        </div>
...
function blog_comments_post($logid,$ckname,$ckmail,$ckurl,$verifyCode,$allow_remark){
	...
	<div class="comment-post" id="comment-post">
		...
		<form method="post" name="commentform" action="<?php echo BLOG_URL; ?>index.php?action=addcom" id="commentform">
			...
			<p>
				<input type="text" name="comname" maxlength="49" value="<?php echo $ckname; ?>" size="22" tabindex="1" style="
    width: 160px;
">
				<label for="author"><small>昵称</small></label>
			</p>
			<p>
				<input type="text" name="commail"  maxlength="128"  value="<?php echo $ckmail; ?>" size="22" tabindex="2" style="
    width: 160px;
">
				<label for="email"><small>邮件地址 (选填)</small></label>
			</p>
			<p>
				<input type="text" name="comurl" maxlength="128"  value="<?php echo $ckurl; ?>" size="22" tabindex="3" style="
    width: 160px;
">
...
<?php //所有分类的名称、别名、发布文章数、描述
function sysort(){
global $CACHE;
$sort_cache = $CACHE->readCache('sort');
foreach($sort_cache as $value){?>
<a href="<?php echo Url::sort($value['sid']); ?>"><?php echo $value['sortname'];?></a> 
<?php echo strtoupper($value['alias']);?> (<?php echo $value['lognum'] ?>)<br />
...
function Get_newlogs($log_num) {
    $db = MySql::getInstance();
    $sql = 	"SELECT gid,title,date,content,views FROM ".DB_PREFIX."blog WHERE type='blog' and hide='n' and sortid IN (1,2) ORDER BY `date` DESC LIMIT 0,$log_num";
    $list = $db->query($sql);
